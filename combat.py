...
class CombatManager:
    ...
    def start_combat(self):
        ...
        while self.player.is_alive() and self.enemy.is_alive():
            ...
            self.player_turn()
            p_msgs = self.player.process_turn_end() # Decr & Tick at end of action
            for m in p_msgs: type_text(m, delay=0.01)
            
            if not self.enemy.is_alive(): break

            if self.skip_enemy_turn:
                type_text(f"{YELLOW}Fate intervenes. {self.enemy.name} is frozen.{RESET}")
                self.skip_enemy_turn = False
            else:
                self.enemy_turn()
    ...
    def player_turn(self):
        ...
        if choice == 1: # Attack
            ...
            self.player.add_effect(Effect("Defend", 1, def_mod=30, color=BLUE))
            type_text("You take a defensive stance.")
        else:
            return self.player_turn()
